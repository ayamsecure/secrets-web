...
if [ ! -d "${VAULT_FOLDER}" ]; then
    ...
    pushd "${VAULT_FOLDER}"
        ...
        git remote add vaultwarden https://github.com/ayamsecure/secrets-web-source.git
    popd
else
	...
	then
           echo "adding ayamsecure/secrets-web-source as remote repository"
           git remote add vaultwarden https://github.com/ayamsecure/secrets-web-source.git
        elif [ "${VAULTWARDEN_REMOTE}" != "https://github.com/ayamsecure/secrets-web-source.git" ]
	then
            echo "Warning: \`git remote get vaultwarden\` did not return the expected repository"
	    echo "expected: https://github.com/ayamsecure/secrets-web-source.git"
